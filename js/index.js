...
var calc = (function () {
    ...
    // Maximum number of hosts for subnet mask
    ...
    // Calculate the required subnet mask from the number of hosts
...
function isValidPrefixLenght(prefixLenght) {
    var reg = /^((?:[1-9])|(?:[1-2][0-9])|(?:3[0-1]))$/
    return reg.test(prefixLenght);
}

function isValidSubnet(prefixLenght) {
    var reg = /^\d+$/
    return reg.test(prefixLenght) && prefixLenght>0 && prefixLenght<=2**(32-$("#netmask").val())-2;

}

function a() {

    $("#inser").after("<button id='generate' class='btn btn-outline-secondary' type='button'>Submit</button>");
...
$(document).ready(function () {
    ...
    $("#generate").blur(function () {
        ...
        for (var i = 1; i <= n; i++) {
            ...
            Subnet "+i+"</span></div><input type='text' class='form-control' \
            placeholder='Size' id='subnet" + i + "' aria-label='Username' aria-describedby='basic-addon1'></div>"
            $("#insert").append(data);
        }
        ...
        <div class="col-md-4">\
            <center>\
                <button id="ready" class="btn" type="button" onclick="main()" >Submit</button>\
            </center>\
        </div>\
        <div class="col-md-4"></div>\
    </div>'
    ...
    $("#network").on("input", function () {
        if (isValidIP($("#network").val())) {
            ...
            if (k)
                $("#ipvaliditon").append("<div class='invalid-feedback'>Please enter the correct IP</div>")
            k = false;
        }
    });
    $("#netmask").on("input", function () {
        if (isValidPrefixLenght($("#netmask").val())) {
            $("#netmask").removeClass("is-invalid")
            $("#netmask").addClass("is-valid")
        }
        else {
            $("#netmask").removeClass("is-valid")
            $("#netmask").addClass("is-invalid")
            if (k)
                $("#netmaskvaliditon").append("<div class='invalid-feedback'>Please enter the correct prefix length</div>")
            k = false;
        }
    });
    $("#generate").on("input", function () {
        if (isValidSubnet($("#generate").val())) {
            $("#generate").removeClass("is-invalid")
            $("#generate").addClass("is-valid")
        }
        else {
            $("#generate").removeClass("is-valid")
            $("#generate").addClass("is-invalid")
            if (k)
                $("#numberSubnetvaliditon").append("<div class='invalid-feedback'>Please enter the correct number of subnet</div>")
            k = false;
        }
    });
